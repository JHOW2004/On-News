...
const PublicProfilePage: React.FC<PublicProfilePageProps> = ({ 
  ...
  return (
    <div>
      <button onClick={onBack} className="mb-4 flex items-center text-primary hover:underline">
      ...
      <div className="space-y-6">
        {activities.length === 0 ? (
          <div className="text-center py-8 text-gray-500">Este usuário ainda não interagiu com notícias.</div>
        ) : (
          activities.map((item, idx) => {
            ...
            let dateLabel = "";
            try {
                dateLabel = format(item.date, "dd/MM/yyyy 'às' HH:mm", { locale: ptBR });
            } catch (e) {
              ...
              <div key={`${item.type}-${idx}`} className="relative">
                ...
                <div className="pl-6">
                  <div className="mb-2 text-sm text-gray-500 dark:text-gray-400">
                    {item.type === 'like' ? 'Curtiu' : 'Comentou'} em {dateLabel}
                  </div>
                  {item.type === 'comment' && (
